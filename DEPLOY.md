...
3. Render shows the **suite** web service and the **suite-db** database. Choose a plan for each (paid plans for real use: free plans sleep and free databases are deleted after a trial period).
4. When asked for **SUITE_PASSWORD**, enter the password from step 1.
5. Click **Apply**. The first deploy takes a few minutes.
6. Open the address Render shows for **suite** (for example `https://suite.onrender.com`) and sign in with the email **owner** and your password.
...
In Render, open **suite**, go to **Settings**, then **Custom Domains**, and add for example `suite.yourcompany.com`. Render shows the DNS record to add at your domain provider. After it works, update `SUITE_URL` in every connected app.

## Changing the owner password

In Render, open **suite**, go to **Environment**, change **SUITE_PASSWORD**, and save. The app restarts and the owner login is signed out everywhere. Other logins and app keys are not affected.
